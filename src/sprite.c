...
static BOOL SpriteList_InitSprite(const SpriteList* list, const SpriteResourcesHeader* resourceData, Sprite* sprite, HeapID heapID);
static CellAnimType SpriteResourcesHeader_GetCellType(const SpriteResourcesHeader* resourceData);
...
static void Sprite_CreateCellAnim(Sprite* sprite, HeapID heapID);
static void Sprite_CreateVRamCellAnim(const SpriteResourcesHeader* resourceData, Sprite* sprite, HeapID heapID);
static void Sprite_CreateMultiCellAnim(Sprite* sprite, HeapID heapID);
...
Sprite* SpriteList_AddAffine(const AffineSpriteListTemplate* template) {
	...
	NNS_G2dSetRndCoreAffineOverwriteMode(&template->list->renderer->rendererCore, sprite->affineOverwriteMode);
	NNS_G2dSetRndCoreFlipMode(&template->list->renderer->rendererCore, sprite->flip & NNS_G2D_RENDERERFLIP_H, sprite->flip & NNS_G2D_RENDERERFLIP_V);
...
void Utility_Clear2DMainOAM(HeapID heapID) {
	...
	memset(oam, 0xC0, sizeof(GXOamAttr) * MAX_SPRITES);
...
static void Sprite_CreateCellAnim(Sprite* sprite, HeapID heapID) {
	CellAnimationData* cellAnim = (CellAnimationData*)&sprite->animData;
	NNS_G2dInitCellAnimation(&cellAnim->anim, NNS_G2dGetAnimSequenceByIdx(cellAnim->animBank, 0), cellAnim->cellBank);
}
...
static u32 GetPaletteIndexForProxy(const NNSG2dImagePaletteProxy* paletteProxy, u32 vramType) {
	...
	return paletteSize != 0 ? NNS_G2dGetImagePaletteLocation(paletteProxy, vramType) / paletteSize : 0;
}
...
static void SpriteList_DrawSprite(const SpriteList* list, Sprite* sprite) {
	...
	if (sprite->affineOverwriteMode == NNS_G2D_RND_AFFINE_OVERWRITE_NONE) {
		NNS_G2dSetRndCoreFlipMode(&list->renderer->rendererCore, sprite->flip & NNS_G2D_RENDERERFLIP_H, sprite->flip & NNS_G2D_RENDERERFLIP_V);
	} else {
