...
void PokemonSummaryScreen_LoadMonSprite(PokemonSummaryScreen* summaryScreen) {
	...
	if (summaryScreen->data->dataType == SUMMARY_DATA_BOX_MON) {
		BoxPokemon_BuildSpriteTemplate(&spriteTemplate, monData, 2);
	} else {
		Pokemon_BuildSpriteTemplate(&spriteTemplate, monData, 2);
	}
	
	if (gIsDiamondPearl) {
		Pokemon_LoadSpriteAnimationFrames(summaryScreen->monSprite.frames, summaryScreen->monData.species);
		summaryScreen->monSprite.flip = SpeciesData_GetSpeciesValue(summaryScreen->monData.species, SPECIES_DATA_FLIP_SPRITE) ^ 1;
	} else {
		PokemonSprite_LoadAnimFramesPt(summaryScreen->monSprite.frames, summaryScreen->monData.species, 1);
...
void PokemonSummaryScreen_LoadMonAnimation(PokemonSummaryScreen* summaryScreen) {
	if (summaryScreen->monData.isEgg) {
		if (gIsDiamondPearl) {
			Pokemon_LoadSpriteAnimation(
				summaryScreen->monSprite.monAnimMan,
				summaryScreen->monSprite.sprite,
				SPECIES_NONE,
				FACE_FRONT,
				NATURE_HARDY,
				summaryScreen->monSprite.flip,
				0);
		} else {
			PokemonSprite_LoadAnimPt(
				summaryScreen->monSprite.monAnimMan,
				summaryScreen->monSprite.sprite,
				SPECIES_NONE,
				FACE_FRONT,
