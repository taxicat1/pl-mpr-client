...
static void LoadBgGraphics(PokemonSummaryScreen* summaryScreen);
...
const ApplicationManagerTemplate gPokemonSummaryScreenApp = {
	.init	  = PokemonSummaryScreen_Init,
	.main	  = PokemonSummaryScreen_Main,
	.exit	  = PokemonSummaryScreen_Exit,
	.overlayID = FS_OVERLAY_ID_NONE
};


BOOL PokemonSummaryScreen_ShouldShowContestData(SaveData* saveData) {
...
static int PokemonSummaryScreen_Init(ApplicationManager* appMan, int* state) {
	...
	summaryScreen->data = ApplicationManager_GetArgs(appMan);
	...
	// New with MPR
	MPRMenu_LoadNetworkIconPalette(NNS_G2D_VRAM_TYPE_2DMAIN, 448, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	MPRMenu_LoadNetworkIconGraphic(NNS_G2D_VRAM_TYPE_2DMAIN, HEAP_ID_POKEMON_SUMMARY_SCREEN);
...
static int PokemonSummaryScreen_Main(ApplicationManager* appMan, int* state) {
	PokemonSummaryScreen* summaryScreen = ApplicationManager_GetData(appMan);
...
static void LoadBgGraphics(PokemonSummaryScreen* summaryScreen) {
	NarcID narc;
	if (gIsDiamondPearl) {
		narc = NARC_INDEX_DP_GRAPHIC__PST_GRA;
	} else {
		narc = NARC_INDEX_PL_GRAPHIC__PL_PST_GRA;
	}
	
	Graphics_LoadTilesToBgLayer(narc, tiles_main_NCGR, summaryScreen->bgConfig, BG_LAYER_MAIN_3, 0, 0, FALSE, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	Graphics_LoadTilemapToBgLayer(narc, page_info_NSCR, summaryScreen->bgConfig, BG_LAYER_MAIN_3, 0, 0, FALSE, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	Graphics_LoadPalette(narc, tiles_main_NCLR, PAL_LOAD_MAIN_BG, 0, 0, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	Graphics_LoadTilemapToBgLayer(narc, move_info_NSCR, summaryScreen->bgConfig, BG_LAYER_MAIN_2, 0, 0, FALSE, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	Graphics_LoadTilesToBgLayer(narc, sub_buttons_NCGR, summaryScreen->bgConfig, BG_LAYER_SUB_0, 0, 0, FALSE, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	Graphics_LoadTilesToBgLayer(narc, tiles_sub_NCGR, summaryScreen->bgConfig, BG_LAYER_SUB_1, 0, 0, FALSE, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	Graphics_LoadTilemapToBgLayer(narc, tiles_sub_NSCR, summaryScreen->bgConfig, BG_LAYER_SUB_1, 0, 0, FALSE, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	Graphics_LoadPalette(narc, tiles_sub_NCLR, PAL_LOAD_SUB_BG, 0, 0, HEAP_ID_POKEMON_SUMMARY_SCREEN);
}
...
static void InitializeStringsAndCopyOTName(PokemonSummaryScreen* summaryScreen) {
	...
	summaryScreen->msgLoader           = MessageLoader_Init(MSG_LOADER_PRELOAD_ENTIRE_BANK, narc, TEXT_BANK_UNIFIED_POKEMON_SUMMARY_SCREEN, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	summaryScreen->ribbonLoader        = MessageLoader_Init(MSG_LOADER_LOAD_ON_DEMAND, narc, TEXT_BANK_UNIFIED_RIBBONS, HEAP_ID_POKEMON_SUMMARY_SCREEN);
	...
	summaryScreen->moveNameLoader      = MessageLoader_Init(MSG_LOADER_PRELOAD_ENTIRE_BANK, narc, TEXT_BANK_UNIFIED_MOVE_NAMES, HEAP_ID_POKEMON_SUMMARY_SCREEN);
...
static int WaitSummaryScreenTransition(PokemonSummaryScreen* summaryScreen) {
	if (IsScreenFadeDone() == TRUE) {
		// New with MPR
		if (summaryScreen->data->isRanchMon) {
			MPRComm_SetFocusedMonIndex(summaryScreen->data->monIndex + (summaryScreen->data->boxIndex * MAX_MONS_PER_BOX));
		} else {
...
static int HandleInput_MoveSwap(PokemonSummaryScreen* summaryScreen) {
	...
	if (JOY_NEW(PAD_BUTTON_A)) {
		...
		if (summaryScreen->cursor != LEARNED_MOVES_MAX && summaryScreen->cursor != summaryScreen->cursorTmp) {
			...
			// New with MPR
			summaryScreen->data->movesSwapped = TRUE;
		} else {
			Sound_PlayEffect(SEQ_SE_DP_DECIDE);
		}
...
static int SetupPoffinFeedConditionPage(PokemonSummaryScreen* summaryScreen) {
	if (JOY_NEW(PAD_BUTTON_A | PAD_BUTTON_B)) {
		...
		if (summaryScreen->data->dataType == SUMMARY_DATA_BOX_MON) {
			boxMon = PokemonSummaryScreen_GetMonData(summaryScreen);
			mon = Pokemon_New(HEAP_ID_POKEMON_SUMMARY_SCREEN);
			Pokemon_FromBoxPokemon(boxMon, mon);
		} else {
			mon = PokemonSummaryScreen_GetMonData(summaryScreen);
		}
		...
		LoadMessageBoxGraphics(summaryScreen->bgConfig, BG_LAYER_MAIN_1, 1024 - (18 + 12), 13, Options_GetFrame(summaryScreen->data->options), HEAP_ID_POKEMON_SUMMARY_SCREEN);
...
static void SetMonDataFromMon(PokemonSummaryScreen* summaryScreen, Pokemon* mon, PokemonSummaryMonData* monData) {
	...
	MessageLoader_GetString(summaryScreen->msgLoader, POKEMON_SUMMARY_SCREEN_SpeciesNameTemplate, summaryScreen->string);
	...
	MessageLoader_GetString(summaryScreen->msgLoader, POKEMON_SUMMARY_SCREEN_NicknameTemplate, summaryScreen->string);
	...
	MessageLoader_GetString(summaryScreen->msgLoader, POKEMON_SUMMARY_SCREEN_OTNameTemplate, summaryScreen->string);
	...
	} else if (Pokemon_IsInfectedWithPokerus(mon) == TRUE) {
...
static void PlayMonCry(PokemonSummaryScreen* summaryScreen) {
	...
	if (summaryScreen->monData.species == SPECIES_CHATOT) {
		Sound_PlayChatotCry(summaryScreen->data->chatotCry, 0, 100, 0);
	} else {
		// BUG: form is not considered for cries as it should be
		Sound_PlayPokemonCry(summaryScreen->monData.species);
	}
}
...
static void LoadCurrentPageTilemap(PokemonSummaryScreen* summaryScreen) {
	...
	void* memberBuffer = NARC_AllocAndReadWholeMemberByIndexPair(narc, memberIndex, HEAP_ID_POKEMON_SUMMARY_SCREEN);
...
static void DrawHealthBar(PokemonSummaryScreen* summaryScreen) {
	u16 baseTile;
	switch (HealthBar_CalcColor(summaryScreen->monData.curHP, summaryScreen->monData.maxHP, 48)) {
		case BARCOLOR_MAX:
		case BARCOLOR_GREEN:
		case BARCOLOR_EMPTY:
...
static void DrawExperienceProgressBar(PokemonSummaryScreen* summaryScreen) {
	u32 maxExp = summaryScreen->monData.nextLevelExp - summaryScreen->monData.curLevelExp;
	u32 curExp = summaryScreen->monData.curExp - summaryScreen->monData.curLevelExp;
...
void* PokemonSummaryScreen_GetMonData(PokemonSummaryScreen* summaryScreen) {
	switch (summaryScreen->data->dataType) {
		case SUMMARY_DATA_MON:
	...
	switch (summaryScreen->pageState) {
		...
		case PAGE_STATE_SCROLL_FINISHED:
			...
			if (summaryScreen->data->mode != SUMMARY_MODE_LOCK_MOVES) {
				PokemonSummaryScreen_ClearAndPrintButtonPrompt(summaryScreen, POKEMON_SUMMARY_SCREEN_MoveSwitch);
...
static void UpdateAppealHearts(PokemonSummaryScreen* summaryScreen, u32 move) {
	...
	if (move != SUMMARY_MOVE_NONE) {
		...
		numHearts = ContestMove_GetAppealPoints(effect) / POINTS_PER_APPEAL_HEART;
...
// This function is never reached because the select move mode doesn't handle subscreen input
static void SetupMoveInfoFromSubscreenButton(PokemonSummaryScreen* summaryScreen) {
	PokemonSummaryScreen_ShowMove5OrCancel(summaryScreen);
	UpdateMoveAttributes(summaryScreen);
	PokemonSummaryScreen_ShowMonIcon(summaryScreen);
...
static u8 SetupRibbonInfo(PokemonSummaryScreen* summaryScreen) {
	switch (summaryScreen->pageState) {
		...
		case PAGE_STATE_SCROLL_FINISHED:
			PokemonSummaryScreen_ClearAndPrintButtonPrompt(summaryScreen, POKEMON_SUMMARY_SCREEN_RibbonSelectCancel);
			PokemonSummaryScreen_UpdateAButtonSprite(summaryScreen, &summaryScreen->staticWindows[SUMMARY_WINDOW_BUTTON_PROMPT]);
...
void PokemonSummaryScreen_SetPlayerProfile(PokemonSummary* summary, const TrainerInfo* trainerInfo) {
	summary->OTName = TrainerInfo_GetName(trainerInfo);
	summary->OTID = TrainerInfo_GetID(trainerInfo);
	summary->OTGender = (u8)TrainerInfo_GetGender(trainerInfo);
}
