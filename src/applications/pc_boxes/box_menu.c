...
void BoxMenu_FillYesNo(BoxApplication* boxApp, u32 menuItemIndex) {
	...
	menu->type = BOX_MENU_TYPE_YES_NO;
}
...
void BoxMenu_FillTopLevelMenuItems(BoxApplication* boxApp) {
	...
	BoxMenu_ClearMenuItems(menu);
	menu->type = BOX_MENU_TYPE_POKEMON_OP;
	...
	switch (BoxApp_GetBoxMode(boxApp)) {
		...
		case PC_MODE_RANCH_DEPOSIT:
			BoxMenu_AddMenuItem(menu, BOX_MENU_RANCH_DEPOSIT);
			BoxMenu_AddMenuItem(menu, BOX_MENU_SUMMARY);
			BoxMenu_AddMenuItem(menu, BOX_MENU_MARK);
			BoxMenu_AddMenuItem(menu, BOX_MENU_CANCEL);
			break;
		...
		case PC_MODE_RANCH_WITHDRAW:
			BoxMenu_AddMenuItem(menu, BOX_MENU_RANCH_WITHDRAW);
			BoxMenu_AddMenuItem(menu, BOX_MENU_SUMMARY);
			BoxMenu_AddMenuItem(menu, BOX_MENU_CANCEL);
			break;
	}
...
void BoxMenu_FillHeaderMenu(BoxApplication* boxApp) {
	...
	if (BoxApp_GetBoxMode(boxApp) != PC_MODE_RANCH_WITHDRAW || BoxApp_GetMaxRanchBoxID(boxApp) != 0) {
		BoxMenu_AddMenuItem(menu, BOX_MENU_JUMP);
	}
	
	if (BoxApp_GetBoxMode(boxApp) != PC_MODE_COMPARE && BoxApp_GetBoxMode(boxApp) != PC_MODE_RANCH_WITHDRAW) {
	...
	menu->type = BOX_MENU_TYPE_HEADER;
}
...
void BoxMenu_FillRanchHeaderMenu(BoxApplication* boxApp, BOOL hasMoreThanTenBoxes) {
	...
	if (BoxApp_GetBoxMode(boxApp) == PC_MODE_RANCH_WITHDRAW) {
		if (hasMoreThanTenBoxes) {
			BoxMenu_AddMenuItem(menu, BOX_MENU_RANCH_10_LEFT);
			BoxMenu_AddMenuItem(menu, BOX_MENU_RANCH_10_RIGHT);
		}
		
		BoxMenu_AddMenuItem(menu, BOX_MENU_RANCH_FIRST);
		BoxMenu_AddMenuItem(menu, BOX_MENU_RANCH_LAST);
	}
...
void BoxMenu_FillWallpaperMenu(BoxApplication* boxApp, BoxMenuItem menuItem) {
	...
	menu->type = BOX_MENU_TYPE_WALLPAPER;
...
void BoxMenu_FillWallpaperSelectionMenu(BoxApplication* boxApp, BoxMenuItem menuItem) {
	static const u16 sWallpaperPages[][MAX_BOX_WALLPAPERS_PER_PAGE] = {
	...
	if (menuItem >= BOX_MENU_FIRST_WALLPAPER_PAGE && menuItem <= BOX_MENU_LAST_DEFAULT_WALLPAPER_PAGE) {
		...
		for (i = 0; i < MAX_BOX_WALLPAPERS_PER_PAGE; i++) {
		...
		for (i = 0; i < MAX_UNLOCKABLE_WALLPAPERS; i++) {
			if (PCBoxes_CheckHasUnlockedWallpaper(pcBoxes, i)) {
				if (firstUnlockedWallpaperPageCount) {
					...
					if (++addedWallpaperCount >= MAX_BOX_WALLPAPERS_PER_PAGE) {
						break;
					}
				}
			}
...
void BoxMenu_FillMarkingsMenu(BoxApplication* boxApp) {
	...
	menu->type = BOX_MENU_TYPE_MARKING;
...
static void BoxMenu_AddMenuItem(BoxMenu* menu, BoxMenuItem menuItem) {
	if (menu->totalMenuItems < MAX_BOX_MENU_ITEMS) {
...
BoxMenuItem BoxMenu_GetMenuNavigation(BoxApplication* boxApp) {
	...
	if (JOY_NEW(PAD_BUTTON_A)) {
		if (menu->type != BOX_MENU_TYPE_MARKING && (menu->markings & (1 << menu->selectedMenuItemIndex))) {
