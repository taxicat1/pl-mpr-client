...
#include "fs/data/namein.naix"
#include "fs/msgdata/generic_names.h"
#include "fs/msgdata/naming_screen.h"
...
typedef struct {
	...
	union {
		NamingScreenState  main;
		ChangeCharsState   changeChars;
	} state;
	
	int      currentCharsIdx;
	BgLayer  freeCharsBgLayer;
...
static void NamingScreen_LoadGraphics(NamingScreen* namingScreen);
static void NamingScreen_AnimateChangeChars(
	BgConfig* bgConfig,
	Window* windows,
	ChangeCharsState* statePtr,
	int currentCharsIdx,
	BgLayer* bgLayerPtr,
...
static void NamingScreen_UpdateCharsPriorities(BgConfig* unused0, BgLayer oldBgLayer, VecFx32 unused1[]);
static void NamingScreen_InitializeCharsPosition(VecFx32 charsPosition[], BgLayer freeBgLayer);
static void NamingScreen_LoadObjectGfx(NamingScreen* namingScreen);
static void NamingScreen_InitSprites(NamingScreen* namingScreen);
static void NamingScreen_TransferChars(void);
static void NamingScreen_InitWindows(NamingScreen* namingScreen, ApplicationManager* appMan);
...
static void NamingScreen_PrintCharOnWindowAndOBJ(Window* window, const charcode_t* tmpBuf, u8* pixelBuf, String* string);
static void NamingScreen_LoadKeyboardLayout(charcode_t out[][13], const int index);
static void NamingScreen_PrintLastCharOfEntryBuf(Window* windows, charcode_t* srcChars, int srcCharIdx, charcode_t* charCodeBuf, u8* pixelBuf, String* string);
...
static void NamingScreen_InitializeCharsGraphics(Window* window, u16 bgColor, int currentCharsIdx, TextColor textColor, void* rawCharData);
static void NamingScreen_UpdateCursorSpritePosition(NamingScreen* namingScreen,	NamingScreenDpadMovement dpadMovement);
...
static void NamingScreen_LoadMonIcon(NNSG2dCharacterData* monIconCharData, NNSG2dPaletteData* plttData, Species monSpecies, int monForm);
...
static void* NamingScreen_PrintStringOnWindowAndGetPixelBuffer(Window* window, String* string, u8 fontId, TextColor color);
...
static const NamingScreenSpriteAnim sSpriteAnimations[] = {
	...
	{ .x = 0x16, .y = 0x38, .anim = 0x25, .priority = 2 },
	{ .x = 0x1A, .y = 0x5B, .anim = 0x27, .priority = 0 }
};
...
static const int sPromptTextIndices[] = {
	NAMING_SCREEN_PlayerPrompt,
	NAMING_SCREEN_PokemonPrompt,
	NAMING_SCREEN_BoxPrompt,
	NAMING_SCREEN_FriendNamePrompt,
	NAMING_SCREEN_FriendCodePrompt,
	NAMING_SCREEN_GroupPrompt,
	NAMING_SCREEN_ThanksPrompt,
	NAMING_SCREEN_FriendNamePrompt
};
...
static BOOL NamingScreen_Init(ApplicationManager* appMan, int* state) {
	...
	switch (*state) {
		case 0:
			...
			namingScreen->namingScreenMsgLoader = MessageLoader_Init(MSG_LOADER_PRELOAD_ENTIRE_BANK, narc, TEXT_BANK_UNIFIED_NAMING_SCREEN, HEAP_ID_NAMING_SCREEN_APP);
			namingScreen->genericNamesMsgLoader = MessageLoader_Init(MSG_LOADER_LOAD_ON_DEMAND, narc, TEXT_BANK_UNIFIED_GENERIC_NAMES, HEAP_ID_NAMING_SCREEN_APP);
			...
			NamingScreen_LoadGraphics(namingScreen);
			...
			NamingScreen_LoadObjectGfx(namingScreen);
			NamingScreen_InitSprites(namingScreen);
			NamingScreen_InitWindows(namingScreen, appMan);
			
			// New with MPR
			MPRMenu_LoadNetworkIconPalette(NNS_G2D_VRAM_TYPE_2DSUB, 448, HEAP_ID_NAMING_SCREEN_APP);
			MPRMenu_LoadNetworkIconGraphic(NNS_G2D_VRAM_TYPE_2DSUB, HEAP_ID_NAMING_SCREEN_APP);
		...
		case 1:
			namingScreen = ApplicationManager_GetData(appMan);
			...
			namingScreen->touchscreenIconBlinker = PaletteAnimator_CreateTask(NULL, 1, 12, HEAP_ID_NAMING_SCREEN_APP);
...
static void NamingScreen_LoadMonIcon(NNSG2dCharacterData* monIconCharData, NNSG2dPaletteData* plttData, Species monSpecies, int monForm) {
	GX_LoadOBJ(monIconCharData->pRawData, (22 * 32 + 31) * 32, 32 * 4 * 4);
	u8* rawData = (u8*)plttData->pRawData;
	GX_LoadOBJPltt((void*)(rawData + PokeIconPaletteIndex(monSpecies, monForm, FALSE) * 32), 6 * 32, 32);
}


static BOOL NamingScreen_Main(ApplicationManager* appMan, int* state) {
	NamingScreen* namingScreen = ApplicationManager_GetData(appMan);
	
	// New with MPR
	if (MPRComm_GetErrorFlag()) {
		if (namingScreen->touchscreenIconBlinker) {
			PaletteAnimator_FinishTask(namingScreen->touchscreenIconBlinker);
		}
		...
		case NMS_APP_STATE_RUNNING:
			switch (namingScreen->state.main) {
				case NMS_STATE_PROCESS_INPUTS:
					if (!namingScreen->keyboardCursor.ignoreInput) {
				...
				case NMS_STATE_PRINT_BATTLE_MESSAGE:
					...
					Window_DrawMessageBoxWithScrollCursor(&namingScreen->windows[NMS_WINDOW_PROMPT], FALSE, 32 * 8, 10);
...
static void NamingScreen_UseDefaultName(NamingScreen* namingScreen, NamingScreenArgs* args) {
	if (namingScreen->type == NAMING_SCREEN_TYPE_PLAYER) {
		...
		if (namingScreen->playerGenderOrMonSpecies == GENDER_MALE) {
			string = MessageLoader_GetNewString(namingScreen->genericNamesMsgLoader, RANDOM_NAME(GENERIC_NAME_PlayerMale));
		} else if (namingScreen->playerGenderOrMonSpecies == GENDER_FEMALE) {
			string = MessageLoader_GetNewString(namingScreen->genericNamesMsgLoader, RANDOM_NAME(GENERIC_NAME_PlayerFemale));
		}
		...
		String* string = MessageLoader_GetNewString(namingScreen->genericNamesMsgLoader, GENERIC_NAME_Pearl);
		String_Copy(args->textInputStr, string);
		String_Free(string);
		String_ToChars(args->textInputStr, args->nameInputRaw, 10);
	} else {
...
static BOOL NamingScreen_Exit(ApplicationManager* appMan, int* unusedState) {
	NamingScreen* namingScreen = ApplicationManager_GetData(appMan);
	NamingScreenArgs* namingScreenArgs = (NamingScreenArgs*)ApplicationManager_GetArgs(appMan);
	int i;
...
NamingScreenArgs* NamingScreenArgs_Init(
	...
	args->textInputStr = String_Init(maxChars + 1, heapID);
	args->battleMsgID = 0;
	args->pcBoxes = NULL;
...
static void NamingScreen_ToggleEngineLayers(BOOL enable) {
	...
	GXLayers_EngineBToggleLayers(GX_BLEND_PLANEMASK_OBJ, enable);
}
...
static void NamingScreen_InitCursorsAndChars(NamingScreen* namingScreen, ApplicationManager* appMan) {
	...
	switch (namingScreen->type) {
		case NAMING_SCREEN_TYPE_POKEMON:
			namingScreen->spritesToUpdate[NMS_SPRITE_LOWER_BUTTON] = TRUE;
			break;
		
		case NAMING_SCREEN_TYPE_FRIEND_CODE:
...
static void NamingScreen_PrepareBattleMessage(NamingScreen* namingScreen, ApplicationManager* appMan) {
	String* string = NULL;
	NamingScreenArgs* args = (NamingScreenArgs*)ApplicationManager_GetArgs(appMan);
...
static void NamingScreen_LoadGraphics(NamingScreen* namingScreen) {
	...
	NarcID narcPoketch;
	if (gIsDiamondPearl) {
		narcNameIn = NARC_INDEX_DP_DATA__NAMEIN;
		narcPoketch = NARC_INDEX_DP_GRAPHIC__POKETCH;
	} else {
		narcNameIn = NARC_INDEX_PL_DATA__NAMEIN;
		narcPoketch = NARC_INDEX_PL_GRAPHIC__POKETCH;
	}
	Graphics_LoadPalette(narcNameIn, naming_screen_NCLR, PAL_LOAD_MAIN_BG, 0, 16 * 3 * 2, HEAP_ID_NAMING_SCREEN_APP);
	Graphics_LoadPalette(narcPoketch, 12, PAL_LOAD_SUB_BG, 0, 16 * 2, HEAP_ID_NAMING_SCREEN_APP);
	Bg_MaskPalette(BG_LAYER_SUB_0, 0);
	
	Graphics_LoadTilesToBgLayer(  narcNameIn, naming_screen_main_tiles_NCGR_lz, bgConfig, BG_LAYER_MAIN_2, 0, (32 * 8) * 0x20, TRUE, HEAP_ID_NAMING_SCREEN_APP);
	Graphics_LoadTilemapToBgLayer(narcNameIn, naming_screen_bg_NSCR_lz,		 bgConfig, BG_LAYER_MAIN_2, 0, 32 * 24 * 2,	 TRUE, HEAP_ID_NAMING_SCREEN_APP);
	Graphics_LoadTilesToBgLayer(  narcNameIn, naming_screen_main_tiles_NCGR_lz, bgConfig, BG_LAYER_MAIN_1, 0, 32 * 8 * 0x20,   TRUE, HEAP_ID_NAMING_SCREEN_APP);
	Graphics_LoadTilemapToBgLayer(narcNameIn, naming_screen_chars_bg_0_NSCR_lz, bgConfig, BG_LAYER_MAIN_1, 0, 32 * 14 * 2,	 TRUE, HEAP_ID_NAMING_SCREEN_APP);
	Graphics_LoadTilemapToBgLayer(narcNameIn, naming_screen_chars_bg_1_NSCR_lz, bgConfig, BG_LAYER_MAIN_0, 0, 32 * 14 * 2,	 TRUE, HEAP_ID_NAMING_SCREEN_APP);
	
	Font_LoadScreenIndicatorsPalette(PAL_LOAD_MAIN_BG, PLTT_OFFSET(12), HEAP_ID_NAMING_SCREEN_APP);
	LoadMessageBoxGraphics(namingScreen->bgConfig, BG_LAYER_SUB_0, 32 * 8, 10, Options_GetFrame(namingScreen->options), HEAP_ID_NAMING_SCREEN_APP);
	Font_LoadScreenIndicatorsPalette(PAL_LOAD_SUB_BG, PLTT_OFFSET(12), HEAP_ID_NAMING_SCREEN_APP);
	
	namingScreen->charDataAlloc = Graphics_GetCharData(narcNameIn, naming_screen_unk_tiles_NCGR_lz, TRUE, &namingScreen->charData, HEAP_ID_NAMING_SCREEN_APP);
}
...
static void NamingScreen_LoadObjectGfx(NamingScreen* namingScreen) {
	...
	SetSubScreenViewRect(&namingScreen->g2dRenderer, 0, 320 * FX32_ONE);
	...
	NarcID narc;
	if (gIsDiamondPearl) {
		narc = NARC_INDEX_DP_DATA__NAMEIN;
	} else {
		narc = NARC_INDEX_PL_DATA__NAMEIN;
	}
	namingScreen->spriteResources[0][SPRITE_RESOURCE_CHAR] = SpriteResourceCollection_AddTiles(
	...
	namingScreen->spriteResources[0][SPRITE_RESOURCE_PLTT] = SpriteResourceCollection_AddPalette(
	...
	namingScreen->spriteResources[0][SPRITE_RESOURCE_CELL] = SpriteResourceCollection_Add(
	...
	namingScreen->spriteResources[0][SPRITE_RESOURCE_ANIM] = SpriteResourceCollection_Add(
	...
	if (namingScreen->type == NAMING_SCREEN_TYPE_POKEMON) {
		NarcID narcPokeIcon;
		if (gIsDiamondPearl) {
			narcPokeIcon = NARC_INDEX_DP_POKETOOL__ICONGRA__POKE_ICON;
		} else {
			narcPokeIcon = NARC_INDEX_PL_POKETOOL__ICONGRA__PL_POKE_ICON;
		}
	...
	namingScreen->spriteResources[1][SPRITE_RESOURCE_CHAR] = SpriteResourceCollection_AddTiles(
		namingScreen->spriteResourceCollections[SPRITE_RESOURCE_CHAR],
		narc,
	...
	namingScreen->spriteResources[1][SPRITE_RESOURCE_PLTT] = SpriteResourceCollection_AddPalette(
		namingScreen->spriteResourceCollections[SPRITE_RESOURCE_PLTT],
		narc,
		naming_screen_sprites_NCLR,
		FALSE,
	...
	namingScreen->spriteResources[1][SPRITE_RESOURCE_CELL] = SpriteResourceCollection_Add(
		namingScreen->spriteResourceCollections[SPRITE_RESOURCE_CELL],
		narc,
	...
	namingScreen->spriteResources[1][SPRITE_RESOURCE_ANIM] = SpriteResourceCollection_Add(
		namingScreen->spriteResourceCollections[SPRITE_RESOURCE_ANIM],
		narc,
...
static void NamingScreen_InitSprites(NamingScreen* namingScreen) {
	...
	for (i = 0; i < namingScreen->maxChars; i++) {
		...
		spriteTemplate.position.y = FX32_ONE * (4 * 8 + 5);
	...
	switch (namingScreen->type) {
		...
		case NAMING_SCREEN_TYPE_POKEMON:
			...
			if (namingScreen->monGender != GENDER_NONE) {
				...
				template->position.y = FX32_CONST(25);
...
static void NamingScreen_AnimateChangeChars(
	...
	BgLayer bgLayer = *bgLayerPtr;
	BgLayer oldBgLayer = bgLayer ^ 1;
...
void NamingScreen_InitWindows(NamingScreen* namingScreen, ApplicationManager* appMan) {
	Window_Add(
		namingScreen->bgConfig,
		&namingScreen->windows[NMS_WINDOW_KEYBOARD_0],
		BG_LAYER_MAIN_0,
	...
	if (namingScreen->type == NAMING_SCREEN_TYPE_POKEMON) {
		...
		namingScreen->currentCharsIdx = 1;
		NamingScreen_LoadKeyboardLayout(namingScreen->keyboardChars, 1);
		NamingScreen_InitializeCharsGraphics(
			&namingScreen->windows[NMS_WINDOW_KEYBOARD_1],
			0x707,
			1,
	...
	} else if (namingScreen->type == NAMING_SCREEN_TYPE_FRIEND_CODE) {
		Graphics_LoadTilemapToBgLayer(
			narc,
	...
	ApplicationManager_GetArgs(appMan);
...
static void NamingScreen_MoveCursor(NamingScreen* namingScreen, NamingScreenDpadMovement dpadMovement) {
	int newX, newY;
	u16 prevKey;
	...
	while (
		...
		if (
			namingScreen->keyboardCursor.prevY == 0 &&
			namingScreen->keyboardChars[newY][newX] == NMS_CONTROL_SKIP &&
...
static void NamingScreen_UpdateCursorSpritePosition(NamingScreen* namingScreen, NamingScreenDpadMovement dpadMovement) {
	if (namingScreen->keyboardCursor.y != 0) {
		...
		int buttonID = namingScreen->keyboardChars[namingScreen->keyboardCursor.y][namingScreen->keyboardCursor.x] - NMS_BUTTON_PAGE_UPPER;
...
static void NamingScreen_PrintCharOnWindowAndOBJ(
	...
	for (i = 0; i < 4; i++) {
		FontOAM_CopyFromWindow(&windows[3], 4, 2, 4 * i, 0, (char*)pixelBuf);
...
static int NamingScreen_ProcessCharacterInput(NamingScreen* namingScreen, charcode_t charCode, BOOL isButtonInput) {
	...
	if (namingScreen->type == NAMING_SCREEN_TYPE_FRIEND_CODE) {
		if (
			charCode == NMS_BUTTON_PAGE_UPPER ||
	...
	switch (charCode) {
		...
		case NMS_BUTTON_OK:
			PaletteAnimator_FinishTask(namingScreen->touchscreenIconBlinker);
			namingScreen->touchscreenIconBlinker = NULL;
			Sprite_SetDrawFlag(namingScreen->uiSprites[NMS_SPRITE_CURSOR], isButtonInput);
