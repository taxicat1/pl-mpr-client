...
#include "fs/poketool/icongra/poke_icon.naix"

#include "fs/poketool/icongra/pokemon_icon_palette.dat" // sPokemonIconPaletteIndex, ICON_*
...
u32 BoxPokemon_IconSpriteIndex(BoxPokemon* boxMon) {
	...
	BOOL isEgg = BoxPokemon_GetValue(boxMon, MON_DATA_IS_EGG, NULL);
...
u32 Pokemon_IconSpriteIndex(Pokemon* mon) {
	return BoxPokemon_IconSpriteIndex(Pokemon_GetBoxPokemon(mon));
}
...
u16 BoxPokemon_IconFormOffset(BoxPokemon* boxMon) {
	u32 speciesOrEgg = BoxPokemon_GetValue(boxMon, MON_DATA_SPECIES_OR_EGG, NULL);
