...
void SoundSystem_Init(ChatotCry* chatotCry, Options* options) {
	...
	NNS_SndArcInit(&soundSys->arc, "data/sound/sound_data.sdat", soundSys->heap, 0);
...
void SoundSystem_SetState(SoundSystemState state) {
	SoundSystem* soundSys = SoundSystem_Get();
...
void* SoundSystem_GetParam(SoundSystemParam param) {
	SoundSystem* soundSys = SoundSystem_Get();
...
NNSSndHandle* SoundSystem_GetSoundHandle(SoundHandleType type) {
	SoundSystem* soundSys = SoundSystem_Get();
...
SoundHandleType SoundSystem_GetSoundHandleTypeFromPlayerID(int playerID) {
	SoundHandleType type;
