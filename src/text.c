...
static RenderResult TextPrinter_Render(TextPrinter* printer);
...
u8 Text_AddPrinter(const TextPrinterTemplate* template, u32 renderDelay, TextPrinterCallback callback) {
	...
	printer->printerTemplate.toPrint.raw = String_GetData(printer->printerTemplate.toPrint.string);
		...
		switch (TextPrinter_Render(printer)) {
			...
			case RENDER_UPDATE:
				if (printer->callback != NULL) {
					printer->callbackResult = (printer->callback)(&printer->printerTemplate, printer->callbackParam);
				}
				return;
...
static RenderResult TextPrinter_Render(TextPrinter* printer) {
	...
	do {
		result = Font_RenderText(printer->printerTemplate.fontID, printer);
...
void Text_DecompressGlyph(const u8* src, u8* dst) {
...
static u8* Text_LoadScreenIndicatorGfx(void) {
	...
	void* ncgr = Graphics_GetCharData(narc, screen_indicators_NCGR, FALSE, &g2dCharData, HEAP_ID_SYSTEM);
...
void Text_RenderScreenIndicator(TextPrinter* printer, u16 unusedX, u16 unusedY, u16 indicator) {
	Window* window = printer->printerTemplate.window;
