#ifndef MPR_POKEMON_SUMMARY_SCREEN_SUMMARY_SCREEN_MAIN_H
#define MPR_POKEMON_SUMMARY_SCREEN_SUMMARY_SCREEN_MAIN_H
...
} SummaryExtraWindowInfo;

typedef enum {
	SUMMARY_WINDOW_MEMO = 0,
	SUMMARY_MEMO_WINDOW_MAX
} SummaryExtraWindowMemo;
...
} SummaryExtraWindowContestMoves;
...
typedef struct {
	...
	u8                 mode;
	u8                 monMax;
	u8                 monIndex;  // 0-Index of the position of the Pokemon within the box or party
	u8                 pageFlags;
	u8                 selectedMoveSlot;
	u8                 returnMode;
	u16                move;
	...
	u32                  dexMode;
	SpecialRibbonsSave*  specialRibbons;
	Poffin*              poffin;
	ChatotCry*           chatotCry;
	BOOL                 showContest;
	
	// New with MPR
	BOOL  movesSwapped;   //<! If the DS side has swapped moves, which should be sent back to Ranch for updating
	BOOL  isRanchMon;     //<! If the Pokemon is stored on the Ranch and not the player's PC
	u8    boxIndex;       //<! Which box (PC or Ranch) the Pokemon is in (0-indexed)
} PokemonSummary;
...
typedef struct {
	Camera*              camera;
	void*                spriteManager;
	SpriteAnimFramePt    frames[MAX_ANIMATION_FRAMES];
...
#endif /* MPR_POKEMON_SUMMARY_SCREEN_SUMMARY_SCREEN_MAIN_H */
